...
export class MemoryStorage implements IStorage {
  ...
  async createOlt(olt: InsertOlt): Promise<Olt> {
    const newOlt: Olt = {
      ...
      capacity: olt.capacity ?? 0, // Default to 0 if not provided
  ...
  async createOdf(odf: InsertOdf): Promise<Odf> {
    const newOdf: Odf = {
      ...
      totalPorts: odf.totalPorts ?? 0, // Default to 0 if not provided
  ...
  async createFat(fat: InsertFat): Promise<Fat> {
    const newFat: Fat = {
      ...
      totalPorts: fat.totalPorts ?? 8, // Default to 8 ports
  ...
  async createAtb(atb: InsertAtb): Promise<Atb> {
    const newAtb: Atb = {
      ...
      totalPorts: atb.totalPorts ?? 4, // Default to 4 ports
  ...
  async createClosure(closure: InsertClosure): Promise<Closure> {
    const newClosure: Closure = {
      ...
      fiberCount: closure.fiberCount ?? 12, // Default to 12 fibers
